    ...
    watch: { },
     setup(props) {
        ...
        function gotoTable() {
          ...
          fetch('http://127.0.0.1:5000/page-view?service_name=' + props.service + '&page_name=system', {
              method: 'GET',
        ...
        function gotoResource(elem) {
          ...
          console.log(elem)
          // TODO: move to its own shared function
          fetch('http://127.0.0.1:5000/page-view?service_name=' + props.service + '&page_name=system&system_name=' + elem, {
              method: 'GET',
              headers: { 'Content-Type': 'application/json', 'login-info': 'get-from-here'}
          }).then(response => response.json())
          .then(payload => page_payload.value = payload);
